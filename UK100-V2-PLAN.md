...
- `fetchCtraderData`: extend the existing UK100 D_1 window from 22 → **32 days**, and add a GBPUSD D_1 fetch over the same window (one extra sequential call). Compute Pearson correlation of the paired **daily close-to-close returns** over the most recent 20 overlapping days → `fx.gbpFtseCorr20d: number | null` (round 2 dp). Extract as a pure function `pearson(xs: number[], ys: number[]): number | null` (null when <15 pairs) with unit tests (known fixture: perfectly inverse series → −1).
