...
def _analyse_proxy(key: str, cfg: dict, macro: dict, us500_gex=None) -> None:
    ...
    # Export to dashboard
    try:
        _export_proxy_dashboard_data(key, spot_live, macro, structure, us500_gex)
    except Exception as e:
        print(f"\n  [Dashboard export skipped: {e}]")
...
def _export_dashboard_data(key: str, spot: float, gex, oi, macro: dict,
    ...
    if gex.gex_by_strike is not None and not gex.gex_by_strike.empty:
        ...
        gex_vals    = [round(float(v) / 1e9, 4) for v in gex.gex_by_strike["net_gex"].tolist()]
    ...
    payload = {
        ...
        "metrics": {
            "net_gex":        round(gex.total_gex, 3),
            "call_gex":       round(gex.call_gex, 3),
            "put_gex":        round(gex.put_gex, 3),
            "regime":         gex.regime,
