...
- **Position sizing** — identical £/point spread-bet model as the swing skill (see `AgentSkill.md` Step 5 for the full worked example and formula). Show the working. **Convert `stake (£/point)` to `create_order`'s `volume` via `SizingReference.md`'s formula: `volume = (stake_per_point ÷ point_size) × 100`** — not a flat ×100 (confirmed wrong for point_size≠1.00; see the 2026-06-12 Crude_SB incident in `BUILD-LOG.md`). Flag `[SIZING UNVERIFIED — see SizingReference.md]` in the trade card for any point_size class not yet confirmed in that table.
