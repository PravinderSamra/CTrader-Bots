...
*V1's whiteboard version of the same architecture (reconciled 2026-07-19):* the uptrend-inducement diagram draws the retail POIs (labeled "BOS", "FVG", "OB" in red) stacked in the **lower half of the up-leg** — the OB at the origin low, the FVG mid-retracement — with circles on the broken high and the origin low (`../03-images/Video1/shot_037.jpg` [5:52]). The full trap diagram labels the seller POI "OB / POI" in **green** at the top, circles each respected high, draws the **blue box around the internal-lows congestion above the pool line** (V1's trap zone), and marks the induced break with a red arrow + red "BOS" at the circled internal high before price breaks below the lows (`shot_039.jpg` [9:38], `shot_038.jpg` [10:52], `shot_034.jpg`). The sell-side execution diagram draws the entry zone as a **box hung off the swept high's wick** — top at the wick extreme, bottom at the swept level, extended rightward — with the red entry arrow leaving from *inside* the box, and repeats the same box fractally on an LTF panel (`shot_046.jpg` [30:34], `shot_045.jpg` [30:54], `shot_042.jpg` [31:04]).
...
- *Measured breathing room (V2 gold shots):* 1m entry — LB wick low ~5,047.3, stop placed 5,046.2–5,046.9 → **~$0.5–1 below the wick (~0.01–0.02% of price)** (`../03-images/Video2/shot_008.jpg`, `shot_026.jpg`); 5m entry #1 — left-hand low wick ~4,025, stop 4,022.07 → **~$3 below (~0.07%)** (`shot_012.jpg`). So "a little breathing room" on gold CFD in practice = roughly $1–3 beyond the wick, scaling with the anchor's timeframe.
...
|---|---|---|---|---|
| 1 | V1 [41:28]–[52:21] | YM micro (MYM Jun '25), 30m→5m, Wed 16 Apr '25 | Textbook short. **From shots:** 30m equal-lows line ~39,450–39,500; Asia purple box ~40,200–40,440; red trap box on the stabbed pre-open lows 40,200–40,270; pre-open equal-highs line 40,600; 9:30 news spike (arc) to 40,717 = first sweep; sell entry at the ~11:00 re-sweep of the 40,600 highs; ride to ~39,600 same day, 39,2xx next (`../03-images/Video1/shot_015.jpg` [43:50], `shot_002.jpg`, `shot_017.jpg` [46:01], `shot_025.jpg` [47:28]). Position-tool RR not shown on screen. | 1:3.6 to first pool, ~1:10 HTF (V1 [50:00]) |
| 2 | V1 [53:56]–[61:32] | EURUSD 4H/daily (Feb–Apr 2025) | Bias lockout + draw prioritization: only buys below the identified low; nothing below it but a huge gap. **From shots:** *the* low = the late-March '25 double-bottom ~1.0765–1.078 (both lows circled on the 4H); the "next internal low all the way down" = the early-March origin lows ~1.04–1.05 (`shot_004.jpg` [57:35]). | n/a (analysis) |
| 3 | V1 [63:28]–[70:08] | NQ micro (MNQ Jun '25) 15m→5m→1m, Thu 10 Apr '25 | Short after news spike sweeps the high. **From shots:** 08:30 news spike to 19,007; swept level ~18,958 (line from the 07:30 high); entry zone = gray box 18,958–19,007; entry 18,958.25, stop +49.00 pts at 19,007.25 (a tick above the spike), partial −88.50 pts at 18,869.75 (trapped-buyer lows), **on-screen RR 1.81**; deeper pool box 18,732–18,770 below (`shot_012.jpg` [1:08:26], `shot_003.jpg`, `shot_032.jpg` [1:09:24]). | 1.81 to first lows |
| 4 | V1 [72:24]–[77:31] | NQ micro (MNQ Jun '25) 5m→1m, Tue Apr 1 '25 | Long: multi-respected lows swept, entry off prior-day no-liquidity wick zone. **From shots:** respected-lows line ~19,283 (tapped 4×); prior-day (Mar 28–31) no-liquidity zone = blue band ~19,210–19,270; 09:30 sweep dipped into the band top; entry 19,270.00, stop 19,206.50 (−63.5), partial 19,392.50 (+122.5, the internal high), **on-screen RR 1.93**; runner to 19,592 (`shot_006.jpg` [1:13:59], `shot_019.jpg` [1:16:48], `shot_031.jpg` [1:17:01], `shot_028.jpg` [1:17:20]). | 1.93 to first partial |
