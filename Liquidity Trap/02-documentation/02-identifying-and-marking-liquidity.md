...
1. **Respected high (sell-side of the pattern / buy stops above).** A high that approached a previous high, respected it (didn't trade through), and moved away → liquidity above (Video 1, [3:22], [13:13]; Video 2, [25:49]–[26:39]). Multiple respects strengthen it (Video 2, [67:20]; Video 3, [13:29]: "taps into it once, twice, three times — this is inducing buyers").
   - *Visual confirmation (Video 3 shots):* "respect" does **not** require a touch of the exact prior price. In V3's 4H example the respect target is an **area** (red box, top edge 20,751.75 extending several hundred points down), and the "respecting" high printed at 20,545.75 — ~200 points *below* the box top, i.e. the wick terminated inside the previously-tapped zone (`../03-images/Video3/shot_003.jpg` [4:42], `shot_005.jpg` [4:46], `shot_014.jpg` label 20,545.75). In the 1H example at V3 [8:01] the respecting high printed 20,839.75 against a marked high of 20,899.00 — a ~59-point (~0.28%) undershoot, no touch (`shot_016.jpg`). So respect = wick terminating at/inside the prior level's zone without closing through it; tolerance observed on NQ ranged from ~60 pts against a line to ~200 pts inside a marked area. *(Video 2 corroboration, gold:* the two "respecting" swing highs printed ~5,104–5,106 against a left-hand high of ~5,110 — a ~4–6 pt (~0.1%) undershoot, no touch, both tips inside the lower half of the later-drawn 5,095.2–5,109.8 zone — `../03-images/Video2/shot_023.jpg` [32:10], `shot_018.jpg` [33:36].) Across both videos the observed undershoot band is ~0.1–0.3% of price, and a touch is never required.*
...
7. Prioritization rules-of-thumb extracted from the examples:
   - **A pool sitting directly above/below with nothing beyond it** ("the next internal low is all the way down here and there's a huge gap in between") is high priority — the market lacks fuel to go the other way (Video 1, [58:09]–[59:11], EU example). *Visual: the example's "only buys below here" low is the late-March 2025 EURUSD 4H double-bottom at ~1.0765–1.078, circled twice on the chart; the "huge gap" runs from there down to the early-March origin lows ~1.04–1.05 (`../03-images/Video1/shot_004.jpg` [57:35]).*
...
9. Drop to the execution TF (5m futures intraday; 1H/4H for swing). Mark:
   ...
   - *Observed legend correction (Video 2 shots):* on his live charts Marco actually uses **red boxes for every marked zone regardless of role** — the equal-highs *target* pool (`../03-images/Video2/shot_018.jpg`), engineered-liquidity zones (`shot_008.jpg`, `shot_020.jpg` — labeled "Eng LQ"), tapped/trap areas (`shot_013.jpg`), and the pre-entry buildup (`shot_015.jpg`) — with thin gray/black **lines** for single levels (target line 4,109.5, internal-lows lines) and no blue markings at all (blue on the whiteboard is reserved for entry/target annotations, `shot_031.jpg`). Only Video 3 consistently reserves blue boxes for LBs. So treat *text labels and role*, not color, as the semantic key when reading Video 2 charts; the red=trap / blue=LB convention files 02/03 adopt is Video 3's, which is the cleanest of the three.
   - *Video 1's observed legend (both surfaces, reconciled 2026-07-19):* **whiteboard** — black/blue = price and structure, red = retail POI labels (BOS/FVG/OB), the seller POI box is **green** ("OB / POI"), and the blue box wraps the *internal-lows congestion above the pool* (the trap zone, confirming A5's reading) (`../03-images/Video1/shot_034.jpg`, `shot_039.jpg`). **Live charts** — **purple boxes** = session/context zones (Asia box, overnight consolidations), **red/pink boxes** = trap and POI zones, **gray box** = the entry zone at a swept high, **blue bands** = no-liquidity entry zones (the Apr-1 prior-day band) plus the blue position tool (`shot_015.jpg` [43:50], `shot_003.jpg`, `shot_019.jpg` [1:16:48]). Three videos, three legends — role and labels, never color, carry the meaning.
   - *Line vs box (when is a pool a price vs a band):* across the reconciled shots, a pool whose constituent wick tips are spread gets a **box spanning the tips** (V2 equal highs, tips ~5,104–5,110 → box 5,095.2–5,109.8; V3 tapped-lows chain → one wide box), while a pool defined by a single tip or near-identical tips gets a **line snapped to the extreme tip** (V3 weekly equal highs → line 23,364.25; V2 internal lows → line). LBs are always drawn as boxes spanning the sweep wick (V3 blue, V2 red/implicit).
