...
- The data client needs your cTrader bearer token:
  `export CTRADER_MCP_TOKEN="<your token>"` (the token in the repo guide is a
  stale demo token — use your own account's). Endpoint and account details are
  in the repo's `ctrader-mcp-integration-guide.md`.
- Everything runs read-only over `scripts/ctrader_http.py` (persistent keep-alive
  HTTP — the reliable path on phone; do **not** use the `mcp__ctrader__*` tools,
  which expire on iPhone/browser).
