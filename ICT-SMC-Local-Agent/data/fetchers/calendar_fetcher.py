...
Currency mapping (which instruments are affected) uses this agent's
instrument names from config/settings.py (e.g. "OIL" not "USOIL",
"US30" not "DOW", "GOLD" not "XAUUSD").
"""
...
# Map event currencies to the instruments they affect (names match INSTRUMENTS
# in config/settings.py).
CURRENCY_TO_SYMBOLS: Dict[str, List[str]] = {
    "USD": ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "USDCAD", "AUDUSD", "NZDUSD",
            "GOLD", "SILVER", "OIL", "BRENT", "NATGAS", "SPX", "NDX", "US30"],
    "EUR": ["EURUSD", "EURGBP", "EURJPY", "EURCAD", "DAX", "FRA40", "EUSTX50"],
    "GBP": ["GBPUSD", "EURGBP", "GBPJPY", "GBPAUD", "GBPCAD", "UK100"],
    "JPY": ["USDJPY", "EURJPY", "GBPJPY", "AUDJPY", "JPN225"],
    "CHF": ["USDCHF"],
    "AUD": ["AUDUSD", "AUDJPY", "GBPAUD", "AUS200"],
    "CAD": ["USDCAD", "EURCAD", "GBPCAD"],
    "NZD": ["NZDUSD"],
    "CNY": ["AUDUSD", "OIL", "GOLD", "AUS200", "HK50"],  # Chinese data moves commodities
    "HKD": ["HK50"],
    "ALL": ["EURUSD", "GBPUSD", "USDJPY", "GOLD", "OIL", "SPX", "NDX", "US30"],
}

ET_ZONE = ZoneInfo("America/New_York")

# Process-lifetime cache — the calendar feed updates weekly, and a single
# report scan calls these functions once per instrument. Avoids redundant
# HTTP fetches within the same run.
_events_cache: Dict[bool, List[dict]] = {}
...
def fetch_events(include_next_week: bool = False) -> List[dict]:
    ...
    """
    if include_next_week in _events_cache:
        return _events_cache[include_next_week]
    ...
    _events_cache[include_next_week] = events
    return events
