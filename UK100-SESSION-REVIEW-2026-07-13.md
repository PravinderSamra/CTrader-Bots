...
| F8 | **European-tape driver (§4A, supersedes V2 Phase D's GER40-only spec).** Add **Euro Stoxx 50** (`EUSTX50`, symbolId **124**, pipDigits 5) to `KNOWN_SYMBOL_IDS`/`PIP_DIGITS` and to the UK100 fetch (spot + day% + a short intraday series for correlation); keep **GER40** (110) as the second tape. New snapshot block `europeanTape`: `{ eurostoxx50DayPct, dax40DayPct, ftseDaxCorr20d, ftseSx5eCorr20d, tapeAgreement: 'ALIGNED'|'SPLIT'|'DIVERGING', preOpenLead: 'UP'|'DOWN'|'NONE' }` (reuse the V2 `pearson()`; `preOpenLead` from the European tape's own overnight-range break at UK100 fetch time when mode=PRE_OPEN/OPENING_HOUR). Bias engine: add a **European-tape driver**, *time-of-day weighted* — **weight 2.5 pre-14:30 London, decaying to 1.0 post-14:30** as the US tape takes over (user-set 2026-07-13; pairs with F2a/F3). Implement as a linear ramp over the 14:30→15:30 London hour (2.5 held until 14:30, interpolate 2.5→1.0 across 14:30–15:30, hold 1.0 after) rather than a hard step, so the driver doesn't lurch at exactly 14:30. This is a **provisional constant to be recalibrated from F7 resolver data**, not a permanent value — leave a `// PROVISIONAL (user-set 2026-07-13): recalibrate from resolver outcomes` comment at the weight. Skill STEP 5 gains a European-tape paragraph (cite `europeanTape`, use the divergence/lead reads per §4A(a)–(c), never as a naive "tape up ⇒ buy"). New macro tile `Uk100EuropeanTapeTile`. Unit-test the correlation + agreement/divergence classification (pure fns). Gold untouched. | `xauusd-dashboard/scripts/lib/ctrader.ts`, `fetch-uk100-data.ts`, `src/types/uk100.ts`, a new tile + explainer + test, README | 2.5–3.5h | P1 |
...
4. **European-tape weighting (F8):** ✅ **ANSWERED 2026-07-13 — weight 2.5 pre-14:30 London, decaying to 1.0 post-14:30** (implemented as a linear ramp across the 14:30–15:30 hour; provisional, recalibrate from F7 resolver data). Still open sub-question: keep CAC 40 (`FRA40`, 125) out (near-collinear with Euro Stoxx 50) unless you specifically want a third confirmation tape — default is out.
