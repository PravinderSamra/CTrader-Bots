...
namespace cAlgo.Robots
{
    ...
    public class IchimokuRsiSwingBot : Robot
    {
        ...
        [Parameter("Timeframe", Group = "Timeframe", DefaultValue = BotTimeFrame.H1)]
        public BotTimeFrame BotTimeFrameParam { get; set; }

        // ══════════════════════════════════════════════════════════════════
        // GROUP C — Ichimoku Settings
        // ══════════════════════════════════════════════════════════════════
        ...
        [Parameter("Session Start Time", Group = "Session", DefaultValue = "00:00:00")]
        public string SessionStartTimeStr { get; set; }

        [Parameter("Session End Time", Group = "Session", DefaultValue = "00:05:00")]
        public string SessionEndTimeStr { get; set; }

        [Parameter("Trading Start Time", Group = "Session", DefaultValue = "00:05:00")]
        ...
        protected override void OnStart()
        {
            ...
            _selectedBars = MarketData.GetBars(GetSelectedTimeFrame());
            _ichimoku = Indicators.IchimokuKinkoHyo(_selectedBars, TenkanPeriod, KijunPeriod, SenkouSpanBPeriod);
            _rsi      = Indicators.RelativeStrengthIndex(_selectedBars.ClosePrices, RsiPeriod);
            if (StopLossMethodParam == StopLossMethod.ATR)
                _atr = Indicators.AverageTrueRange(_selectedBars, AtrPeriod, MovingAverageType.Exponential);

            _selectedBars.BarClosed += OnSelectedBarClosed;
            _lastSignalBarIndex = -1;
            Log("TIMEFRAME: {0}", BotTimeFrameParam);
            ...
            Log("ICHIMOKU SETTINGS: Tenkan={0} Kijun={1} SpanB={2} Displacement={3}",
                TenkanPeriod, KijunPeriod, SenkouSpanBPeriod, CloudDisplacement);
            Log("TIP — Intraday alternatives: M5=7/20/40 | M15=9/22/44 | H1=9/26/52 | H4=9/26/52");
        ...
        // Subscribed to _selectedBars.BarClosed for multi-timeframe support
        // ══════════════════════════════════════════════════════════════════

        private void OnSelectedBarClosed(BarClosedEventArgs args)
        {
            ...
            if ((InstrumentTypeParam == InstrumentType.Stocks || InstrumentTypeParam == InstrumentType.Indices)
                && _atr != null && _selectedBars.Count > 2)
            {
                double atrVal    = _atr.Result[1];
                double gapSize   = Math.Abs(_selectedBars.OpenPrices[1] - _selectedBars.ClosePrices[2]);
        ...
        // ══════════════════════════════════════════════════════════════════
        // 7.0 TIMEFRAME SELECTION
        // ══════════════════════════════════════════════════════════════════

        private TimeFrame GetSelectedTimeFrame()
        {
            switch (BotTimeFrameParam)
            {
                case BotTimeFrame.M5:  return TimeFrame.Minute5;
                case BotTimeFrame.M15: return TimeFrame.Minute15;
                case BotTimeFrame.H1:  return TimeFrame.Hour;
                case BotTimeFrame.H4:  return TimeFrame.Hour4;
                case BotTimeFrame.D1:  return TimeFrame.Daily;
        ...
        private string GetBotLabel(DateTime sessionDate, int tradeNum)
        {
            return string.Format("{0}_{1}_{2}_{3:D2}",
                BotLabelPrefix, SymbolName, sessionDate.ToString("yyyyMMdd"), tradeNum);
        }

        private int GetTradesExecutedForSessionDate(DateTime sessionDate)
        {
            // Match all labels for this day: prefix = ICHI_SYMBOL_YYYYMMDD_
            string dayPrefix = string.Format("{0}_{1}_{2}_",
                BotLabelPrefix, SymbolName, sessionDate.ToString("yyyyMMdd"));
            var ids = new HashSet<long>();

            foreach (var pos in Positions)
            {
                if (pos == null || pos.SymbolName != SymbolName) continue;
                if (string.IsNullOrEmpty(pos.Label) || !pos.Label.StartsWith(dayPrefix)) continue;
                if (IsIgnorableDustPosition(pos)) continue;
                ids.Add(pos.Id);
            }

            // Search history for each numbered label this day
            int maxSearch = Math.Max(MaxTradesPerDay, 1) * 3;
            for (int i = 1; i <= maxSearch; i++)
            {
                string label = string.Format("{0}{1:D2}", dayPrefix, i);
                try
                {
        ...
        private bool EvaluateLongSignal()
        {
            ...
            if (_selectedBars.Count - 1 == _lastSignalBarIndex) return false;
            ...
            // In CTrader, SenkouSpan[1] = last confirmed bar's cloud value (no displacement offset needed)
            double spanA = _ichimoku.SenkouSpanA[1];
            double spanB = _ichimoku.SenkouSpanB[1];
            ...
            double closePrice  = _selectedBars.ClosePrices[1];
            ...
            if (EnableChikouConfirmation)
            {
                ...
                double historicalClose = _selectedBars.ClosePrices[1 + CloudDisplacement]; // price CloudDisplacement bars before bar[1]
        ...
        private bool EvaluateShortSignal()
        {
            ...
            if (EnableChikouConfirmation)
            {
                ...
                double historicalClose = _selectedBars.ClosePrices[1 + CloudDisplacement];
                if (chikouClose >= historicalClose)
                {
        ...
        private double CalculateStopLoss(TradeType tradeType, double entryEstimate)
        {
            ...
            switch (StopLossMethodParam)
            {
                ...
                case StopLossMethod.SwingHighLow:
                    if (tradeType == TradeType.Buy)
                    {
                        double swingLow = _selectedBars.LowPrices[1];
                        for (int i = 1; i <= SwingLookbackBars && i < _selectedBars.Count; i++)
                            if (_selectedBars.LowPrices[i] < swingLow) swingLow = _selectedBars.LowPrices[i];
                        slPrice = swingLow - SlSwingBufferPips * Symbol.PipSize;
                    }
                    else
                    {
                        double swingHigh = _selectedBars.HighPrices[1];
                        for (int i = 1; i <= SwingLookbackBars && i < _selectedBars.Count; i++)
                            if (_selectedBars.HighPrices[i] > swingHigh) swingHigh = _selectedBars.HighPrices[i];
        ...
        private void EnterTrade(TradeType tradeType)
        {
            ...
            // In CTrader, SenkouSpan[1] = last confirmed bar's cloud value
            double spanA        = _ichimoku.SenkouSpanA[1];
            double spanB        = _ichimoku.SenkouSpanB[1];
            double tenkan       = _ichimoku.TenkanSen[1];
            double kijun        = _ichimoku.KijunSen[1];
            ...
            string label  = GetBotLabel(_currentSessionDate, _tradesToday + 1);
            ...
            _lastSignalBarIndex = _selectedBars.Count - 1;
