...
- **Use the `orb` block, not an Asian range:** `overnight_high`/`overnight_low` (22:00 prev day → 08:00 cash open — has it been swept?), `orb_high`/`orb_low` (the 08:00–08:15 range), `orb_broken_direction` (UP/DOWN/none — the direction of the first post-08:15 M5 close outside the range).
  - **When the engine's `orb.orb_broken_direction` disagrees with the macro snapshot's `orbContext.orbBrokenDirection`, trust the ENGINE.** Both use the identical "first post-08:15 M5 close outside the range, chronological, sticky" rule (`build_orb` in `uk100_sessions.py` == `firstCloseOutside` in `fetch-uk100-data.ts`), so a disagreement is never a definitional conflict — it is purely a freshness gap: the snapshot is computed hourly and can be up to ~60 min stale, so it may not yet have seen a break the engine (run live at scan time) already has (observed 2026-07-23: engine `UP` vs snapshot `NONE`, ~40 min apart). Report the engine's value; if you mention the snapshot's, label it as the earlier-in-the-session read, don't present the two as a contradiction.
...
- **Expansion vs range read (`orbContext.expansionState`):** cite it directly — it compares today's realised range (`adrUsedPct`) against a per-hour baseline of how much of a typical UK100 day's RANGE and VOLUME is done by this hour (range front-loads, volume back-loads — ~67% of range but only ~47% of volume is typically done by 11:00 BST). Fields: `label` (`EXPANSION` = ahead of the typical pace / trend day · `COMPRESSED` = below pace, coiled or dead · `NORMAL` · `UNKNOWN` pre-open or no baseline), `rangeVsTypical` (>1 = ahead of pace), `expectedVolPctByNow` (volume time-budget), and **`moveMostlyDone`** (true = most of the day's range/volume already spent → expect a rangebound remainder). **Do NOT read a high `adrUsedPct` as "the move is done" on its own** — that is exactly the time-blind mistake this fixes: if `moveMostlyDone` is false (heavy afternoon volume still to come), an afternoon expansion is still live even when the morning range looks large (observed 2026-07-23: 59% ADR used at 09:53 but only ~37% of volume done — the day then expanded down in the afternoon). Report `expansionState.note` verbatim in the LIQUIDITY MAP / SESSION CONTEXT.
...
- else `TREND_EXPECTED` — `|bias.score| ≥ 5` AND the gap (`orbContext.gapPct`) is in the same direction as the bias AND the overnight range is comfortably below the ADR14 norm (i.e. the move hasn't already happened overnight). **Corroborate with `expansionState`:** an `EXPANSION` label (or `rangeVsTypical > 1.35`) with `moveMostlyDone` false supports `TREND_EXPECTED`; if `expansionState.moveMostlyDone` is true, the trend move has largely already happened — downgrade to `RANGE_EXPECTED` regardless of the bias score.
- else `RANGE_EXPECTED` (baseline) — the default, and the forced choice whenever `expansionState.moveMostlyDone` is true.
...
- **`expansionState.moveMostlyDone` → tighten or stand aside:** when `orbContext.expansionState.moveMostlyDone` is true (most of the day's range AND volume typically already spent), keep targets to the nearest liquidity pool only and lean toward `STAND_ASIDE` / `WAIT` for a fresh setup — the remainder is likely rangebound. Conversely, when `label` is `EXPANSION` with `moveMostlyDone` false (afternoon volume still to come), a trend-continuation target beyond the immediate pool is defensible. This is the time-aware refinement of the raw ADR cap above — prefer it when both are present.
...
- **T1 must sit BEYOND the trigger level, never ON it (mandatory).** When the trigger is a break of a level (e.g. "short on an M5 close below the ORB low 10642.8"), that level is the *entry pivot*, not a target — the first target must be the next liquidity pool **past** it in the trade's direction (the next SSL/BSL, PDL/PDH, or a measured move), and `drawOnLiquidity` must equal that next pool, not the break level. Setting `T1` == the break/trigger level ships a trade that is "already at target" the moment it triggers (observed 2026-07-23: draw/T1 and the trigger were both 10642.8; the genuine draw below was PDL 10557.2, which price approached to 10581). If the only pool below the trigger is beyond the remaining-range cap, say so and mark it a stretch target rather than pinning T1 to the trigger.
- **Invalidation/stop needs a buffer above/below the structural shelf, not flush on it.** Never set `invalidation` (or `tradeIdea.stop`) exactly at the ORB high/low or a BOS level price is currently oscillating around — a 1-pt overshoot close will false-trip it right before the thesis works (observed 2026-07-23: invalidation 10707 was tagged by a 10708.3 close at 10:00 BST, then price reversed and the bearish thesis played out to −94pts). Place invalidation beyond the *next* structural level with a small buffer (e.g. above the overnight high, or the shelf + ~0.05–0.1× ADR), the same "+ buffer" the TRADE IDEA stop already uses — keep the orbPlaybook `invalidation` and the `tradeIdea.stop` consistent with each other.
...
- **PLAN** — ORB playbook direction + day type + the single actionable sentence of the day, consistent with STEP 8 (e.g. "BOTH_OK half size — prefer the fade toward 10502 over chasing PDH; WAIT status, US_OVERLAP gate active"). When `expansionState.label` is `EXPANSION` or `expansionState.moveMostlyDone` is true, fold that into this bullet (e.g. "range 90% done + most volume in — expect a rangebound remainder, tighten targets" or "expansion day, afternoon volume still to come — trend-continuation favoured").
