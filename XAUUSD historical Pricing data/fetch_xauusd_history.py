#!/usr/bin/env python3
"""
Bulk XAUUSD historical OHLCV fetcher — cTrader over direct HTTPS.
...
# ── file paths ────────────────────────────────────────────────────────────────
def _paths(out_dir: str, period: str):
    raw = os.path.join(out_dir, f"_raw_bars_{period}.jsonl")
    ckpt = os.path.join(out_dir, f"_checkpoint_{period}.json")
    return raw, ckpt
...
def fetch(args):
    ...
    raw_path, ckpt_path = _paths(out_dir, args.period)
...
def finalize(args):
    ...
    out_dir = args.out_dir
    raw_path, _ = _paths(out_dir, args.period)
    ...
    for ts in sorted(seen):
        ...
        o = b.get("open", b.get("close", 0)) / _PIP
        c = b.get("close", b.get("open", 0)) / _PIP
        row = (dt.strftime("%Y-%m-%dT%H:%M:%SZ"),
               round(o, 5), round(b["high"] / _PIP, 5), round(b["low"] / _PIP, 5),
